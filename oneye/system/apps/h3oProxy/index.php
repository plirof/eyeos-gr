...
/*
   ...
   Modified to run under eyeOS by Thorsten Zoerner <me@thorsten-zoerner.com>
*/

//error_reporting(E_ALL);

include_once("apps/h3oProxy/libproxy.eyecode");
//
...
$GLOBALS["_hosts"]             = array
                    (
                        '#^127\.|192\.169\.|10\.|172\.(1[6-9]|2[0-9]|3[01])\.|localhost#i'
                    );
...
//$GLOBALS["_script_url"]        = 'http' . ((isset($_ENV['HTTPS']) && $_ENV['HTTPS'] == 'on') || $_SERVER['SERVER_PORT'] == 443 ? 's' : '') . '://' . $GLOBALS["_http_host"] . ($_SERVER['SERVER_PORT'] != 80 && $_SERVER['SERVER_PORT'] != 443 ? ':' . $_SERVER['SERVER_PORT'] : '') . $_SERVER['PHP_SELF'];
//$GLOBALS["_script_base"]       = substr($GLOBALS["_script_url"], 0, strrpos($GLOBALS["_script_url"], '/')+1);
$GLOBALS["_url"]               = '';
...
else if (isset($GLOBALS["_cookie"]['flags']) && ctype_alnum($GLOBALS["_cookie"]['flags']))
{
    $GLOBALS["_iflags"] = $GLOBALS["_cookie"]['flags'];
}
...
if ($GLOBALS["_system"]['stripslashes'])
{
    ...
    $GLOBALS["_cookie"] = _stripslashes($GLOBALS["_cookie"]);
}
...
do
{
    ...
    {
        $GLOBALS["_request_headers"] .= "User-Agent: h3oProxy for eyeOS\r\n";
    }
    ...
    if (!empty($GLOBALS["_cookie"]))
    {
        $GLOBALS["_cookie"]  = '';
        $GLOBALS["_auth_creds"]    = array();
    
        foreach ($GLOBALS["_cookie"] as $cookie_id => $cookie_content)
        {
            ...
            if ($cookie_id[0] === 'COOKIE')
            {
                ...
                {
                    $GLOBALS["_cookie"] .= ($GLOBALS["_cookie"] != '' ? '; ' : '') . (empty($cookie_id[1]) ? '' : $cookie_id[1] . '=') . $cookie_content[0];
                }
            }
        ...
        if ($GLOBALS["_cookie"] != '')
        {
            $GLOBALS["_request_headers"] .= "Cookie: ".$GLOBALS["_cookie"]."\r\n";
        }
    }
    ...
    if (isset($GLOBALS["_response_headers"]['set-cookie']) && $GLOBALS["_flags"]['accept_cookies'])
    {
        foreach ($GLOBALS["_response_headers"]['set-cookie'] as $cookie)
        {
            ...
            if (count($GLOBALS["_cookie"]) >= 15 && time()-$expires_time <= 0)
            {
                $GLOBALS["_set_cookie"][] = add_cookie(current($GLOBALS["_cookie"]), '', 1);
            }
...
//		$pi=pathinfo($GLOBALS["_url"]);
		$userAccountDir = um('getCurrentUserDir')."/files/h3oProxy";
		if(!vfs('isdir', array($userAccountDir))) {
				vfs('mkDir', array($userAccountDir));
		}
		$fname=$userAccountDir."/".basename($GLOBALS["_url"]);
		
		vfs('create', array($fname));
		$fp=vfs('open', array($fname, "w"));
//		$fp=fopen("tmp.tmp","w");	
	...
	if(isset($fp)) {
		...
		fclose($fp);
		echo "<a href='javascript:history.back(-1);'>File Saved on your eyeOS Account in folder h3oProxy/".basename($fname)."</a>";
	}
    exit(0);
}
                ...
                case 'form':
                    if (isset($attrs['action']))
                    {
                        ...
                        if (trim($attrs['action']) === '')
                        {
                            ...
                            $extra_html.= '<input type="hidden" name="msg" value="proxy" />';                        
